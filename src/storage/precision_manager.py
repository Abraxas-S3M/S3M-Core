...
class PrecisionManager:
    ...
    def __init__(self, b2: B2Connector) -> None:
        self.b2 = b2
    ...
    def pull_fp16_for_training(self, engine_id: str, local_dir: Path) -> Path:
        ...
        if VaultPaths.is_blocked_engine(engine_id):
            raise ValueError(
                ...
                "Grok-1 (300 GB) stays in BackBlaze as validation oracle."
            )
        remote = VaultPaths.fp16_base(engine_id)
        self.b2.sync_down(remote, local_dir)
    ...
    def push_fp16_adapter(self, engine_id: str, track: str, adapter_dir: Path) -> dict[str, int]:
        """Push FP16 LoRA adapter from RunPod back to BackBlaze."""
        remote = VaultPaths.fp16_adapter(engine_id, track)
        return self.b2.sync_up(adapter_dir, remote)

    def push_fp16_merged(self, engine_id: str, track: str, merged_dir: Path) -> dict[str, int]:
        """Push merged FP16 model from Hetzner to BackBlaze."""
        remote = VaultPaths.fp16_merged(engine_id, track)
        return self.b2.sync_up(merged_dir, remote)
    ...
    def push_q4_serving(self, engine_id: str, gguf_path: Path) -> dict[str, Any]:
        ...
        for key in self.b2.list_keys(remote_prefix):
            self.b2.delete_file(key)
        remote = f"{remote_prefix}{Path(gguf_path).name}"
        return self.b2.upload_file(gguf_path, remote)
    ...
    def promote_merged_to_base(self, engine_id: str, track: str) -> dict[str, int]:
        ...
        keys = self.b2.list_keys(merged_prefix)
        if not keys:
        ...
        with tempfile.TemporaryDirectory(prefix="s3m-promote-") as tmp_dir:
            ...
            for key in keys:
                ...
                self.b2.download_file(key, local_copy)
                target_key = f"{base_prefix}{relative}"
                upload_result = self.b2.upload_file(local_copy, target_key)
                promoted += 1
    ...
    def get_model_inventory(self) -> dict[str, dict[str, Any]]:
        ...
        for engine in ["phi3-medium", "mistral-7b", "allam-7b", "grok-300b"]:
            fp16_keys = self.b2.list_keys(VaultPaths.fp16_base(engine))
            q4_keys = self.b2.list_keys(VaultPaths.q4_serving(engine))
            inventory[engine] = {
                "fp16_files": len(fp16_keys),
                "fp16_size_gb": sum(self.b2.get_file_size(key) for key in fp16_keys) / (1024**3),
                "q4_files": len(q4_keys),
                "q4_size_gb": sum(self.b2.get_file_size(key) for key in q4_keys) / (1024**3),
