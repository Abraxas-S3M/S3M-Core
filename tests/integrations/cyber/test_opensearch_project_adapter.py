...
def _load():
    module = importlib.import_module("packages.integrations.cyber.opensearch-project.adapter")
    return module.OpensearchProjectAdapter, module


def test_manifest_is_loaded_from_yaml() -> None:
    adapter_class, _ = _load()
    manifest = adapter_class(mode="airgapped").get_manifest()
    assert manifest.slug == "opensearch-project"
    assert manifest.domain == "cyber"
    assert manifest.source_url == "https://github.com/opensearch-project"


def test_logger_name_matches_required_pattern() -> None:
    adapter_class, _ = _load()
    adapter = adapter_class(mode="airgapped")
    assert adapter.logger.name == "s3m.integrations.cyber.opensearch-project"
...
def test_execute_airgapped_returns_fixture_payload() -> None:
    adapter_class, _ = _load()
    output = adapter_class(mode="airgapped").execute({"operation": "search_analytics"})
    assert output["source"] == "fixture"
    assert output["result"]["cluster_name"] == "opensearch-cyber-cluster"
