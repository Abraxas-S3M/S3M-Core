...
def _load():
    module = importlib.import_module("packages.integrations.cyber.phantomcyber-playbooks.adapter")
    return module.PhantomcyberplaybooksAdapter, module


def test_manifest_is_loaded_from_yaml() -> None:
    adapter_class, _ = _load()
    manifest = adapter_class(mode="airgapped").get_manifest()
    assert manifest.slug == "phantomcyber-playbooks"
    assert manifest.domain == "cyber"
    assert manifest.source_url == "https://github.com/phantomcyber/playbooks"


def test_logger_name_matches_required_pattern() -> None:
    adapter_class, _ = _load()
    adapter = adapter_class(mode="airgapped")
    assert adapter.logger.name == "s3m.integrations.cyber.phantomcyber-playbooks"
...
def test_execute_airgapped_returns_fixture_payload() -> None:
    adapter_class, _ = _load()
    output = adapter_class(mode="airgapped").execute({"operation": "recommend_playbook"})
    assert output["source"] == "fixture"
    assert output["result"]["recommended_playbook"]["name"] == "Email Phishing Investigation and Containment"
