"""Adapter for generic 2D surveillance radar feeds.

Military context:
Legacy short-range radars often provide only range/azimuth contacts. This
adapter injects nominal elevation assumptions so they still contribute to COP.
"""
...
class Generic2DRadarAdapter(BaseRadarAdapter):
    """Normalize generic 2D radar payloads to typed RadarScan."""
    ...
    def parse_raw_scan(self, raw_scan: Dict[str, Any]) -> RadarScan:
        ...
        scan_mode = ScanMode.from_value(raw_scan.get("scan_mode", "SEARCH"))
        ...
        for idx, entry in enumerate(plots_raw):
            ...
            snr_db = float(entry.get("snr_db", 12.0))
            plot = RadarPlot(
