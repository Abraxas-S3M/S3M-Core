"""Adapter for Ship-Detection-Using-Satellite-Images integration workflows.
...
class ShipDetectionUsingSatelliteAdapter(IntegrationAdapter):
    """S3M adapter for Ship-Detection-Using-Satellite-Images sensor-analytics workflows."""

    integration_id = "ship-detection-using-satellite-images"
    domain = "sensor_analytics"
    _COMMAND_CANDIDATES = ('python3', 'jupyter', 'pip')
    _DEFAULT_OPERATION = "infer_ships_from_planet_imagery"

    def __init__(self, mode: str | None = None) -> None:
        super().__init__(mode=mode)
        self.logger = logging.getLogger("s3m.integrations.sensor_analytics.ship-detection-using-satellite-images")
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            name=str(raw.get("name") or "Ship-Detection-Using-Satellite-Images"),
            slug=str(raw.get("slug") or self.integration_id),
            domain=str(raw.get("domain") or self.domain),
            source_url=str(raw.get("source_url") or "https://github.com/SherinBK/Ship-detection"),
            license=str(raw.get("license") or "Unknown"),
            description=str(
                raw.get("description")
                or "Deep-learning ship detection workflow for Planet satellite imagery supporting maritime domain awareness."
            ),
            integration_type=str(raw.get("integration_type") or "adapter"),
            capabilities=self._coerce_list(
                raw.get("capabilities")
                or ['planet_imagery_detection', 'ship_localization', 'maritime_activity_screening']
            ),
