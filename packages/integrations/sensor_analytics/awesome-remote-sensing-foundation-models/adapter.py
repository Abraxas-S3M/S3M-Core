"""Adapter for Awesome-Remote-Sensing-Foundation-Models integration workflows.
...
class AwesomeRemoteSensingFoundationAdapter(IntegrationAdapter):
    """S3M adapter for Awesome-Remote-Sensing-Foundation-Models sensor-analytics workflows."""

    integration_id = "awesome-remote-sensing-foundation-models"
    domain = "sensor_analytics"
    _COMMAND_CANDIDATES = ('python3', 'git', 'jq')
    _DEFAULT_OPERATION = "catalog_remote_sensing_foundation_models"

    def __init__(self, mode: str | None = None) -> None:
        super().__init__(mode=mode)
        self.logger = logging.getLogger("s3m.integrations.sensor_analytics.awesome-remote-sensing-foundation-models")
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            name=str(raw.get("name") or "Awesome-Remote-Sensing-Foundation-Models"),
            slug=str(raw.get("slug") or self.integration_id),
            domain=str(raw.get("domain") or self.domain),
            source_url=str(raw.get("source_url") or "https://github.com/Jack-bo1220/Awesome-Remote-Sensing-Foundation-Models"),
            license=str(raw.get("license") or "Unknown"),
            description=str(
                raw.get("description")
                or "Curated index of remote-sensing foundation models for planning sovereign SATINT model selection pipelines."
            ),
            integration_type=str(raw.get("integration_type") or "adapter"),
            capabilities=self._coerce_list(
                raw.get("capabilities")
                or ['model_catalog_discovery', 'task_capability_mapping', 'deployment_readiness_tagging']
            ),
