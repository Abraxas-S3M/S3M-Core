"""Adapter for TRACK-MY-VOYAGE integration workflows.
...
class TrackMyVoyageAdapter(IntegrationAdapter):
    """S3M adapter for TRACK-MY-VOYAGE sensor-analytics workflows."""

    integration_id = "track-my-voyage"
    domain = "sensor_analytics"
    _COMMAND_CANDIDATES = ('python3', 'yolo', 'ultralytics')
    _DEFAULT_OPERATION = "detect_and_classify_vessels"

    def __init__(self, mode: str | None = None) -> None:
        super().__init__(mode=mode)
        self.logger = logging.getLogger("s3m.integrations.sensor_analytics.track-my-voyage")
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            name=str(raw.get("name") or "TRACK-MY-VOYAGE"),
            slug=str(raw.get("slug") or self.integration_id),
            domain=str(raw.get("domain") or self.domain),
            source_url=str(raw.get("source_url") or "https://github.com/gopika-10/TRACK-MY-VOYAGE"),
            license=str(raw.get("license") or "Unknown"),
            description=str(
                raw.get("description")
                or "YOLOv8-based maritime vessel detection and classification from satellite and aerial imagery feeds."
            ),
            integration_type=str(raw.get("integration_type") or "adapter"),
            capabilities=self._coerce_list(
                raw.get("capabilities")
                or ['yolov8_ship_detection', 'real_time_classification', 'imagery_inference_pipeline']
            ),
