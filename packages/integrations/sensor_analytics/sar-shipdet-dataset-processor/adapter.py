"""Adapter for SAR-ShipDet-Dataset-Processor integration workflows.
...
class SarShipdetDatasetProcessorAdapter(IntegrationAdapter):
    """S3M adapter for SAR-ShipDet-Dataset-Processor sensor-analytics workflows."""

    integration_id = "sar-shipdet-dataset-processor"
    domain = "sensor_analytics"
    _COMMAND_CANDIDATES = ('python3', 'git', 'pip')
    _DEFAULT_OPERATION = "normalize_sar_ship_datasets"

    def __init__(self, mode: str | None = None) -> None:
        super().__init__(mode=mode)
        self.logger = logging.getLogger("s3m.integrations.sensor_analytics.sar-shipdet-dataset-processor")
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            name=str(raw.get("name") or "SAR-ShipDet-Dataset-Processor"),
            slug=str(raw.get("slug") or self.integration_id),
            domain=str(raw.get("domain") or self.domain),
            source_url=str(raw.get("source_url") or "https://github.com/egshkim/SAR-ShipDet-Dataset-Processor"),
            license=str(raw.get("license") or "Unknown"),
            description=str(
                raw.get("description")
                or "Unified preprocessing pipeline for multiple SAR ship-detection datasets used in maritime ISR model training."
            ),
            integration_type=str(raw.get("integration_type") or "adapter"),
            capabilities=self._coerce_list(
                raw.get("capabilities")
                or ['dataset_schema_unification', 'sar_annotation_normalization', 'train_val_split_generation']
            ),
