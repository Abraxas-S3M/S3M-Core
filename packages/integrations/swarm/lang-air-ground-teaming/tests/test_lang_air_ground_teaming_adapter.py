...
def _load_adapter():
    ...
    return module.LangAirGroundTeamingAdapter


def test_manifest_metadata_is_loaded() -> None:
    adapter_cls = _load_adapter()
    manifest = adapter_cls(mode="airgapped").get_manifest()
    assert manifest.slug == "lang-air-ground-teaming"
...
def test_execute_returns_fixture_when_airgapped() -> None:
    adapter_cls = _load_adapter()
    response = adapter_cls(mode="airgapped").execute({"operation": "language_conditioned_team_plan"})
    assert response["source"] == "fixture"
    assert "teaming_plan" in response["result"]
