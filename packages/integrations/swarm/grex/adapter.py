"""Adapter for grex swarm operations.
...
class GrexAdapter(IntegrationAdapter):
    """S3M swarm adapter for grex."""

    integration_id = "grex"
    domain = "swarm"
    _COMMAND_CANDIDATES = ('grex', 'ros2', 'colcon', 'python3')
    _DEFAULT_OPERATION = "simulate_swarm_mission"

    def __init__(self, mode: str | None = None) -> None:
        super().__init__(mode=mode)
        self.logger = logging.getLogger("s3m.integrations.swarm.grex")
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            name=str(raw.get("name") or "grex"),
            slug=str(raw.get("slug") or self.integration_id),
            domain=str(raw.get("domain") or self.domain),
            source_url=str(raw.get("source_url") or "https://github.com/NU-IDEAS-Lab/grex"),
            license=str(raw.get("license") or "MIT"),
            description=str(raw.get("description") or "Multi-agent framework for ROS 2 enabling plug-and-play simulation of complex agents in MAS."),
    ...
    def execute(self, params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        return {
            ...
            "operation": operation,
            "request": request,
            "note": "GREX runtime is reachable for ROS 2 swarm simulation and mission rehearsal.",
        }
