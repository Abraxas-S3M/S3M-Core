"""Adapter for Snipe-IT maintenance and asset lifecycle workflows.

Military/tactical context:
This wrapper supports equipment lifecycle accountability so sustainment teams
can maintain mission-capable inventories during disconnected operations.
"""
...
class SnipeItAdapter(IntegrationAdapter):
    """S3M adapter for Snipe-IT maintenance and asset tracking."""

    integration_id = "snipe-it"
    domain = "maintenance"
    _COMMAND_CANDIDATES = ("snipeit", "php", "artisan", "docker")
    _DEFAULT_OPERATION = "asset_lifecycle_sync"

    def __init__(self, mode: str | None = None) -> None:
        super().__init__(mode=mode)
        self.logger = logging.getLogger("s3m.integrations.maintenance.snipe-it")
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            ...
            description=str(
                raw.get("description")
                or "Open-source IT asset management wrapper with maintenance lifecycle tracking."
            ),
            integration_type=str(raw.get("integration_type") or "adapter"),
            capabilities=list(
                raw.get("capabilities")
                or ["asset_inventory", "lifecycle_tracking", "maintenance_schedule_audit"]
            ),
