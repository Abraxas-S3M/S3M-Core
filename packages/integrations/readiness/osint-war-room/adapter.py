"""Adapter for OSINT-War-Room personnel readiness workflows.

Military/tactical context:
This wrapper normalizes tactical war-room indicators into personnel readiness
signals for command staff operating in disconnected sovereign environments.
"""
...
class OsintWarRoomAdapter(IntegrationAdapter):
    """S3M adapter for OSINT-War-Room readiness dashboard workflows."""

    integration_id = "osint-war-room"
    domain = "readiness"

    _repo_env_var = "READINESS_OSINT_WAR_ROOM_PATH"
    _module_hints = ("streamlit", "plotly", "pandas")
    _binary_hints = ("python3", "git")
    _default_operation = "personnel_readiness_war_room"
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            name=str(raw.get("name") or "OSINT-War-Room"),
            slug=str(raw.get("slug") or self.integration_id),
            domain=str(raw.get("domain") or self.domain),
            source_url=str(raw.get("source_url") or "https://github.com/Hue-Jhan/OSINT-War-Room"),
            license=str(raw.get("license") or "Unknown"),
            description=str(
                raw.get("description")
                or "Tactical war-room dashboard adapted for personnel readiness and force health monitoring."
            ),
            integration_type=str(raw.get("integration_type") or "adapter"),
            capabilities=self._coerce_list(
                raw.get("capabilities")
                or ["war_room_readiness_monitoring", "shift_fatigue_tracking", "alert_prioritization"]
            ),
