...
def _load_adapter():
    ...
    return module.MilitaryManagementSystemAdapter


def test_manifest_metadata_is_loaded():
    adapter_cls = _load_adapter()
    manifest = adapter_cls(mode="airgapped").get_manifest()
    assert manifest.slug == "military-management-system"
...
def test_execute_returns_fixture_when_airgapped():
    adapter_cls = _load_adapter()
    response = adapter_cls(mode="airgapped").execute({"operation": "personnel_readiness_overview"})
    assert response["source"] == "fixture"
    assert response["result"]["report_id"] == "mms-readiness-2026-04-10-01"
