"""wazuh-suricata integration adapter.
...
class WazuhSuricataAdapter(IntegrationAdapter):
    """Thin wrapper for wazuh-suricata cyber-defense workflows."""
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            ...
            description=str(raw.get("description") or "Dedicated Wazuh and Suricata integration for host-network correlation in tactical SOC monitoring."),
    ...
    def validate_availability(self) -> bool:
        ...
        return bool(
            any(shutil.which(tool) for tool in ("wazuh-agentd", "suricata"))
        or Path("/var/ossec/etc/ossec.conf").exists()
        or Path("/etc/suricata/suricata.yaml").exists()
        )
    ...
    def execute(self, params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        action = str(request.get("action") or "ids_correlation")
        return {
            "status": "accepted",
            "mode": "online",
            "integration_id": self.integration_id,
