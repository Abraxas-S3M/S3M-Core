"""Playbooks integration adapter for defensive cyber operations.

Military/tactical context:
This wrapper lets S3M planners query local incident-response playbooks so SOC teams
can execute repeatable countermeasures during contested operations, even when fully
airgapped from external networks.
"""
...
class PlaybooksAdapter(IntegrationAdapter):
    """Adapter for NIST-aligned SOC incident-response playbook references."""

    integration_id = "playbooks"
    domain = "cyber"
    ...
    def _load_manifest_mapping(self) -> dict[str, Any]:
        ...
        if not isinstance(raw, dict):
            raise ValueError("Playbooks manifest must contain a YAML mapping.")
        return raw
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            name=str(raw.get("name") or "Playbooks"),
            slug=str(raw.get("slug") or self.integration_id),
            domain=str(raw.get("domain") or self.domain),
            source_url=str(raw.get("source_url") or "https://github.com/socfortress/Playbooks"),
            license=str(raw.get("license") or "Unknown"),
            description=str(
                raw.get("description")
                or "NIST-aligned incident-response workflows for SOC defensive operations."
            ),
            integration_type=str(raw.get("integration_type") or "adapter"),
            capabilities=self._coerce_list(
                raw.get("capabilities")
                or ["incident-response", "soc-workflow", "playbook-reference"]
            ),
    ...
    def execute(self, params: dict[str, Any] | None = None) -> dict[str, Any]:
        """Execute tactical playbook lookup for SOC mission support."""
        params = params or {}
