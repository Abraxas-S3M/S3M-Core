"""gsvsoc_cirt-playbook-battle-cards integration adapter.

Military/tactical context:
This wrapper exposes rapid-response battle-card guidance so cyber defenders can
execute immediate countermeasures during active incidents in denied environments.
"""
...
class GsvsocCirtPlaybookBattleAdapter(IntegrationAdapter):
    """Adapter for cyber incident response battle-card references."""

    integration_id = "gsvsoc-cirt-playbook-battle-cards"
    domain = "cyber"
    ...
    def _load_manifest_mapping(self) -> dict[str, Any]:
        ...
        if not isinstance(raw, dict):
            raise ValueError("gsvsoc battle-card manifest must contain a YAML mapping.")
        return raw
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            name=str(raw.get("name") or "gsvsoc_cirt-playbook-battle-cards"),
            slug=str(raw.get("slug") or self.integration_id),
            domain=str(raw.get("domain") or self.domain),
            source_url=str(
                raw.get("source_url")
                or "https://github.com/guardsight/gsvsoc_cirt-playbook-battle-cards"
            ),
            license=str(raw.get("license") or "Unknown"),
            description=str(
                raw.get("description")
                or "Battle cards for rapid cyber incident containment and remediation."
            ),
            integration_type=str(raw.get("integration_type") or "adapter"),
            capabilities=self._coerce_list(
                raw.get("capabilities")
                or ["rapid-countermeasure", "incident-response", "battle-card-reference"]
            ),
    ...
    def execute(self, params: dict[str, Any] | None = None) -> dict[str, Any]:
        """Execute tactical battle-card lookup for rapid SOC action."""
        params = params or {}
        ...
        threat_family = str(params.get("threat_family", "")).strip().lower()
        try:
        ...
        if self.is_airgapped:
            ...
            if threat_family:
                entries = [
                    item
                    for item in entries
                    if threat_family == str(item.get("threat_family", "")).lower()
                ]
            return {
                **(payload if isinstance(payload, dict) else {}),
                ...
                "mode": "airgapped",
                "query": query or None,
                "threat_family": threat_family or None,
        ...
        for file_path in sorted(repo_path.rglob("*.md")):
            ...
            if query and query not in relative_path.lower():
                continue
            if threat_family and threat_family not in relative_path.lower():
                continue
            entries.append(
                {
