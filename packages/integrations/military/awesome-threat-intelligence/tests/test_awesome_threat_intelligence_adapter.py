...
def _load_adapter():
    ...
    return module.AwesomeThreatIntelligenceAdapter


def test_manifest_metadata_is_loaded():
    adapter_cls = _load_adapter()
    manifest = adapter_cls(mode="airgapped").get_manifest()
    assert manifest.slug == "awesome-threat-intelligence"
...
def test_execute_returns_fixture_when_airgapped():
    adapter_cls = _load_adapter()
    response = adapter_cls(mode="airgapped").execute({"operation": "curate_threat_sources"})
    assert response["source"] == "fixture"
    assert response["result"]["intel_brief_id"] == "threat-brief-uav-117"
