...
class AegisAdapter(IntegrationAdapter):
    """S3M integration adapter for Aegis."""

    integration_id = "aegis"
    domain = "military"
    _COMMAND_CANDIDATES = ('python3', 'aegis', 'ros2')

    def __init__(self, mode: str | None = None) -> None:
        super().__init__(mode=mode)
        self.logger = logging.getLogger("s3m.integrations.military.aegis")
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            name=str(raw.get("name", "Aegis")),
            slug=str(raw.get("slug", self.integration_id)),
            domain=str(raw.get("domain", self.domain)),
            source_url=str(raw.get("source_url", "https://github.com/Vitalcheffe/Aegis")),
            license=str(raw.get("license", "Apache 2.0")),
            description=str(raw.get("description", "Autonomous kinetic interceptor swarm logic with sensor fusion for layered short-range air defense drills.")),
            integration_type=str(raw.get("integration_type", "adapter")),
            capabilities=[str(item) for item in raw.get("capabilities", ['swarm-intercept', 'sensor-fusion', 'air-defense-logic'])],
    ...
    def execute(self, params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        operation = str(safe_params.get("operation") or "intercept_rehearsal")
        ...
        if not self.validate_availability():
            return {
                ...
                "operation": operation,
                "status": "unavailable",
                "error": "aegis runtime is not installed or configured",
                "request": safe_params,
            }
