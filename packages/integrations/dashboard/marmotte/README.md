...
S3M dashboard integration wrapper for **Marmotte**.
...
1. A local path environment variable (`MARMOTTE_PATH` or `S3M_MARMOTTE_PATH`), or
...
module = importlib.import_module("packages.integrations.dashboard.marmotte.adapter")
adapter_cls = getattr(module, "MarmotteAdapter")
