...
S3M dashboard integration wrapper for **Awesome-Threat-Detection (Curated)**.
...
1. A local path environment variable (`AWESOME_THREAT_DETECTION_CURATED_PATH` or `S3M_AWESOME_THREAT_DETECTION_CURATED_PATH`), or
...
module = importlib.import_module("packages.integrations.dashboard.awesome-threat-detection-curated.adapter")
adapter_cls = getattr(module, "AwesomeThreatDetectioncuratedAdapter")
