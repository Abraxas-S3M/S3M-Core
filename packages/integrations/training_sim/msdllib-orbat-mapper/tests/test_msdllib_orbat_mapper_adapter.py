...
def _load_adapter():
    ...
    return module.MsdlliborbatMapperAdapter


def test_manifest_metadata_is_loaded():
    adapter_cls = _load_adapter()
    manifest = adapter_cls(mode="airgapped").get_manifest()
    assert manifest.slug == "msdllib-orbat-mapper"
...
def test_execute_returns_fixture_when_airgapped():
    adapter_cls = _load_adapter()
    response = adapter_cls(mode="airgapped").execute({"operation": "parse_military_scenario"})
    assert response["source"] == "fixture"
    assert response["result"]["scenario_name"] == "Border-Sector-Delta-Exercise"
