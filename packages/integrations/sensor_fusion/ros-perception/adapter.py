"""Adapter for ROS Perception stack readiness checks.

Military/tactical context:
This wrapper supports mission sensor fusion by validating perception-stack
availability for sovereign robotic platforms in disconnected deployments.
"""
...
class RosPerceptionAdapter(IntegrationAdapter):
    """S3M wrapper for ros-perception package readiness workflows."""

    integration_id = "ros-perception"
    domain = "sensor_fusion"

    _SUPPORTED_OPERATIONS = {"pipeline_status", "node_probe", "message_contracts"}
    _COMMAND_CANDIDATES = ("ros2", "rospack")
    _ROS_PACKAGES = ("image_pipeline", "perception_pcl", "vision_opencv")
    _DEFAULT_OPERATION = "pipeline_status"

    def __init__(self, mode: str | None = None) -> None:
        super().__init__(mode=mode)
        self.logger = logging.getLogger("s3m.integrations.sensor_fusion.ros-perception")
    ...
    def _sanitize_params(params: dict[str, Any] | None) -> dict[str, Any]:
        """Validate mission payloads before ROS perception checks."""
        if params is None:
            return {}
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            name=str(raw.get("name") or "ros-perception"),
            slug=str(raw.get("slug") or self.integration_id),
            domain=str(raw.get("domain") or self.domain),
            source_url=str(raw.get("source_url") or "https://github.com/ros-perception"),
            license=str(raw.get("license") or "BSD"),
            description=str(
                raw.get("description")
                or "ROS Perception stack components for image and point-cloud sensor fusion."
            ),
            integration_type=str(raw.get("integration_type") or "adapter"),
            capabilities=self._coerce_list(
                raw.get("capabilities") or ["image_pipeline_readiness", "pointcloud_processing", "ros_message_fusion"]
            ),
    ...
    def execute(self, params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if not self.validate_availability():
            return {
                ...
                "operation": operation,
                "request": safe_params,
                "message": "Required local ROS perception dependencies are unavailable.",
            }
