"""Adapter for ROS/ROS2 perception interface package checks.

Military/tactical context:
This wrapper supports autonomous mission interoperability by validating
common perception message interfaces in sovereign edge deployments.
"""
...
class PerceptionInterfacesAdapter(IntegrationAdapter):
    """S3M wrapper for perception_interfaces package readiness workflows."""

    integration_id = "perception-interfaces"
    domain = "sensor_fusion"

    _SUPPORTED_OPERATIONS = {"interface_status", "message_contracts", "tooling_probe"}
    _COMMAND_CANDIDATES = ("ros2", "rospack")
    _ROS_PACKAGES = ("perception_interfaces", "perception_msgs")
    _DEFAULT_OPERATION = "interface_status"

    def __init__(self, mode: str | None = None) -> None:
        super().__init__(mode=mode)
        self.logger = logging.getLogger("s3m.integrations.sensor_fusion.perception-interfaces")
    ...
    def _sanitize_params(params: dict[str, Any] | None) -> dict[str, Any]:
        """Validate mission payloads before perception interface checks."""
        if params is None:
            return {}
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            name=str(raw.get("name") or "perception_interfaces"),
            slug=str(raw.get("slug") or self.integration_id),
            domain=str(raw.get("domain") or self.domain),
            source_url=str(raw.get("source_url") or "https://github.com/ika-rwth-aachen/perception_interfaces"),
            license=str(raw.get("license") or "MIT"),
            description=str(
                raw.get("description")
                or "ROS/ROS2 common messages and tooling for automated-driving perception interoperability."
            ),
            integration_type=str(raw.get("integration_type") or "adapter"),
            capabilities=self._coerce_list(
                raw.get("capabilities") or ["message_interface_validation", "ros2_tooling_probe", "perception_contract_checks"]
            ),
    ...
    def validate_availability(self) -> bool:
        ...
        if any(shutil.which(command) is not None for command in self._COMMAND_CANDIDATES):
            return True
    ...
    def execute(self, params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if not self.validate_availability():
            return {
                ...
                "operation": operation,
                "request": safe_params,
                "message": "Required local perception-interface dependencies are unavailable.",
            }
