"""Adapter for isolation-forest network threat detection workflows.

Military/tactical context:
This wrapper supports contested-network defense by exposing deterministic,
airgapped-safe anomaly triage outputs for tactical SOC operations.
"""
...
class AiDrivenThreatDetectionAdapter(IntegrationAdapter):
    """S3M wrapper for AI-Driven-Threat-Detection-System workflows."""

    integration_id = "ai-driven-threat-detection-system"
    domain = "sensor_fusion"

    _SUPPORTED_OPERATIONS = {"anomaly_score", "traffic_triage", "model_status"}
    ...
    def __init__(self, mode: str | None = None) -> None:
        ...
        self.logger = logging.getLogger("s3m.integrations.sensor_fusion.ai-driven-threat-detection-system")
    ...
    def _sanitize_params(params: dict[str, Any] | None) -> dict[str, Any]:
        """Validate defensive payloads before threat-fusion processing."""
        if params is None:
            return {}
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            name=str(raw.get("name") or "AI-Driven-Threat-Detection-System"),
            slug=str(raw.get("slug") or self.integration_id),
            domain=str(raw.get("domain") or self.domain),
            source_url=str(raw.get("source_url") or "https://github.com/melisa48/AI-Driven-Threat-Detection-System"),
            license=str(raw.get("license") or "MIT"),
            description=str(
                raw.get("description")
                or "Isolation Forest-based network traffic anomaly detection for threat triage."
            ),
            integration_type=str(raw.get("integration_type") or "adapter"),
            capabilities=self._coerce_list(
                raw.get("capabilities")
                or ["network_anomaly_detection", "traffic_baselining", "isolation_forest_scoring"]
            ),
    ...
    def execute(self, params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if not self.validate_availability():
            return {
                ...
                "operation": operation,
                "request": safe_params,
                "message": "Required local threat-detection dependencies are unavailable.",
            }
