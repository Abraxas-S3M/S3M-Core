"""Adapter for nostr-p2p secure communications workflows.
...
class NostrP2pAdapter(IntegrationAdapter):
    """S3M communications adapter for nostr-p2p."""

    integration_id = "nostr-p2p"
    domain = "comms"
    _COMMAND_CANDIDATES = ('nostr-p2p', 'nostr', 'python3')
    _DEFAULT_OPERATION = "relay_message"

    def __init__(self, mode: str | None = None) -> None:
        super().__init__(mode=mode)
        self.logger = logging.getLogger("s3m.integrations.comms.nostr-p2p")
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            name=str(raw.get("name") or "nostr-p2p"),
            slug=str(raw.get("slug") or self.integration_id),
            domain=str(raw.get("domain") or self.domain),
            source_url=str(raw.get("source_url") or "https://github.com/cmdruid/nostr-p2p"),
            license=str(raw.get("license") or "Unknown"),
            description=str(raw.get("description") or "Peer-to-peer messaging protocol over Nostr relays for resilient tactical communications."),
            integration_type=str(raw.get("integration_type") or "adapter"),
            capabilities=self._coerce_str_list(raw.get("capabilities") or ['relay_routed_messaging', 'peer_identity_exchange', 'disconnected_fixture_replay']),
