"""Adapter for chatmail/relay secure communications workflows.
...
class ChatmailrelayAdapter(IntegrationAdapter):
    """S3M communications adapter for chatmail/relay."""

    integration_id = "chatmail-relay"
    domain = "comms"
    _COMMAND_CANDIDATES = ('chatmail-relay', 'docker', 'podman')
    _DEFAULT_OPERATION = "relay_health_status"

    def __init__(self, mode: str | None = None) -> None:
        super().__init__(mode=mode)
        self.logger = logging.getLogger("s3m.integrations.comms.chatmail-relay")
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            name=str(raw.get("name") or "chatmail/relay"),
            slug=str(raw.get("slug") or self.integration_id),
            domain=str(raw.get("domain") or self.domain),
            source_url=str(raw.get("source_url") or "https://github.com/chatmail/relay"),
            license=str(raw.get("license") or "Unknown"),
            description=str(raw.get("description") or "Encrypted chatmail relay wrapper for metadata-minimized message routing in contested environments."),
            integration_type=str(raw.get("integration_type") or "adapter"),
            capabilities=self._coerce_str_list(raw.get("capabilities") or ['encrypted_relay_status', 'metadata_minimized_routing', 'offline_fixture_replay']),
