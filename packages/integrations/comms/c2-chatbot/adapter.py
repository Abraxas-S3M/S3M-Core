"""Adapter for c2_chatbot secure communications workflows.
...
class C2ChatbotAdapter(IntegrationAdapter):
    """S3M communications adapter for c2_chatbot."""

    integration_id = "c2-chatbot"
    domain = "comms"
    _COMMAND_CANDIDATES = ('c2_chatbot', 'python3')
    _DEFAULT_OPERATION = "extract_command_intent"

    def __init__(self, mode: str | None = None) -> None:
        super().__init__(mode=mode)
        self.logger = logging.getLogger("s3m.integrations.comms.c2-chatbot")
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            name=str(raw.get("name") or "c2_chatbot"),
            slug=str(raw.get("slug") or self.integration_id),
            domain=str(raw.get("domain") or self.domain),
            source_url=str(raw.get("source_url") or "https://github.com/traiannicula/c2_chatbot"),
            license=str(raw.get("license") or "Unknown"),
            description=str(raw.get("description") or "C2 chatbot wrapper for extracting structured command intent from natural-language mission traffic."),
            integration_type=str(raw.get("integration_type") or "adapter"),
            capabilities=self._coerce_str_list(raw.get("capabilities") or ['command_intent_extraction', 'rule_based_order_parsing', 'offline_fixture_replay']),
