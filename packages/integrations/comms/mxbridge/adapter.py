"""Adapter for mxbridge secure communications workflows.
...
class MxbridgeAdapter(IntegrationAdapter):
    """S3M comms adapter for mxbridge."""

    integration_id = "mxbridge"
    domain = "comms"
    _COMMAND_CANDIDATES = ('mxbridge',)
    _MODULE_CANDIDATES = ('mxbridge',)
    _DEFAULT_OPERATION = "bridge_route_snapshot"

    def __init__(self, mode: str | None = None) -> None:
        super().__init__(mode=mode)
        self.logger = logging.getLogger("s3m.integrations.comms.mxbridge")
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            name=str(raw.get("name") or "mxbridge"),
            slug=str(raw.get("slug") or self.integration_id),
            domain=str(raw.get("domain") or self.domain),
            source_url=str(raw.get("source_url") or "https://github.com/djmaze/mxbridge"),
            license=str(raw.get("license") or "Unknown"),
            description=str(raw.get("description") or "Matrix-XMPP bridge for interoperable encrypted routing"),
            integration_type=str(raw.get("integration_type") or "adapter"),
            capabilities=list(raw.get("capabilities") or ['matrix_xmpp_interop', 'encrypted_message_relay', 'cross_network_identity_mapping']),
