"""Adapter for libsignal secure communications workflows.
...
class LibsignalAdapter(IntegrationAdapter):
    """S3M comms adapter for libsignal."""

    integration_id = "libsignal"
    domain = "comms"
    _COMMAND_CANDIDATES = ('cargo',)
    _MODULE_CANDIDATES = ('libsignal',)
    _DEFAULT_OPERATION = "session_crypto_readiness"

    def __init__(self, mode: str | None = None) -> None:
        super().__init__(mode=mode)
        self.logger = logging.getLogger("s3m.integrations.comms.libsignal")
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            name=str(raw.get("name") or "libsignal"),
            slug=str(raw.get("slug") or self.integration_id),
            domain=str(raw.get("domain") or self.domain),
            source_url=str(raw.get("source_url") or "https://github.com/signalapp/libsignal"),
            license=str(raw.get("license") or "Unknown"),
            description=str(raw.get("description") or "Official Signal Protocol library for end-to-end encryption"),
            integration_type=str(raw.get("integration_type") or "adapter"),
            capabilities=list(raw.get("capabilities") or ['signal_protocol_sessions', 'forward_secrecy_key_rotation', 'sealed_sender_workflows']),
