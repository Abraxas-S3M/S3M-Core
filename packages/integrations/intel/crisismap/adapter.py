"""Adapter for crisismap intelligence workflows.
...
class CrisismapAdapter(IntegrationAdapter):
    """S3M adapter for crisismap intelligence briefing operations."""

    integration_id = "crisismap"
    domain = "intel"

    _repo_env_var = "INTEL_CRISISMAP_PATH"
    _module_hints = ('folium', 'geopandas', 'pandas')
    _binary_hints = ('python3', 'git')
    _default_operation = "crisis_map_overlay"
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            name=str(raw.get("name") or "crisismap"),
            slug=str(raw.get("slug") or self.integration_id),
            domain=str(raw.get("domain") or self.domain),
            source_url=str(raw.get("source_url") or "https://github.com/realwaynesun/crisismap"),
            license=str(raw.get("license") or "Unknown"),
            description=str(raw.get("description") or "Real-time geopolitical crisis intelligence dashboard with conflict-event geospatial risk overlays."),
            integration_type=str(raw.get("integration_type") or "adapter"),
            capabilities=self._coerce_list(raw.get("capabilities") or ['conflict_mapping', 'event_fusion', 'risk_overlay', 'satellite_context_tags']),
