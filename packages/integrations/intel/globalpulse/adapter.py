"""Adapter for globalpulse intelligence workflows.
...
class GlobalpulseAdapter(IntegrationAdapter):
    """S3M adapter for globalpulse intelligence briefing operations."""

    integration_id = "globalpulse"
    domain = "intel"

    _repo_env_var = "INTEL_GLOBALPULSE_PATH"
    _module_hints = ('streamlit', 'pandas', 'langdetect')
    _binary_hints = ('python3', 'git')
    _default_operation = "global_intel_pulse"
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            name=str(raw.get("name") or "globalpulse"),
            slug=str(raw.get("slug") or self.integration_id),
            domain=str(raw.get("domain") or self.domain),
            source_url=str(raw.get("source_url") or "https://github.com/ntamero/globalpulse"),
            license=str(raw.get("license") or "Unknown"),
            description=str(raw.get("description") or "Real-time global intelligence dashboard with AI analysis and multilingual briefing support."),
            integration_type=str(raw.get("integration_type") or "adapter"),
            capabilities=self._coerce_list(raw.get("capabilities") or ['global_situational_awareness', 'multilingual_analysis', 'alert_prioritization']),
