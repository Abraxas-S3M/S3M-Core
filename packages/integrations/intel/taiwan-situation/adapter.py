"""Adapter for TaiWan-situation intelligence workflows.
...
class TaiwanSituationAdapter(IntegrationAdapter):
    """S3M adapter for TaiWan-situation intelligence briefing operations."""

    integration_id = "taiwan-situation"
    domain = "intel"

    _repo_env_var = "INTEL_TAIWAN_SITUATION_PATH"
    _module_hints = ('streamlit', 'pandas', 'plotly')
    _binary_hints = ('streamlit', 'git')
    _default_operation = "taiwan_strait_brief"
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            name=str(raw.get("name") or "TaiWan-situation"),
            slug=str(raw.get("slug") or self.integration_id),
            domain=str(raw.get("domain") or self.domain),
            source_url=str(raw.get("source_url") or "https://github.com/Pluto114/TaiWan-situation"),
            license=str(raw.get("license") or "Unknown"),
            description=str(raw.get("description") or "Automated Taiwan Strait tension index dashboard with risk modeling and news aggregation."),
            integration_type=str(raw.get("integration_type") or "adapter"),
            capabilities=self._coerce_list(raw.get("capabilities") or ['tension_index', 'risk_modeling', 'news_aggregation', 'escalation_briefing']),
