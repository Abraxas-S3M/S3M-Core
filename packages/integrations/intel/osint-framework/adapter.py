"""Adapter for OSINT-Framework intelligence workflows.
...
class OsintFrameworkAdapter(IntegrationAdapter):
    """S3M adapter for OSINT-Framework intelligence briefing operations."""

    integration_id = "osint-framework"
    domain = "intel"

    _repo_env_var = "INTEL_OSINT_FRAMEWORK_PATH"
    _module_hints = ('yaml',)
    _binary_hints = ('git',)
    _default_operation = "tool_catalog_brief"
    ...
    def get_manifest(self) -> IntegrationManifest:
        ...
        return IntegrationManifest(
            ...
            source_url=str(raw.get("source_url") or "https://github.com/lockfale/OSINT-Framework"),
            license=str(raw.get("license") or "Unknown"),
            description=str(raw.get("description") or "Web-based OSINT framework with categorized reconnaissance and intelligence-gathering tools."),
            integration_type=str(raw.get("integration_type") or "adapter"),
            capabilities=self._coerce_list(raw.get("capabilities") or ['tool_catalog', 'collection_planning', 'source_discovery', 'workflow_bootstrap']),
